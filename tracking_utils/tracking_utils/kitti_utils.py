...
import scipy
import det3.methods.second_tracking.tracking_utils.object3d as object3d
import torch


def get_objects_from_label(label_file, idx):
    lines = [line.rstrip() for line in open(label_file)]
    objects = []
    for i in range(len(lines)):
        frame_i = int(lines[i].split(' ')[0])
        if frame_i == idx:
            objects.append(object3d.Object3d(lines[i]))
    return objects
